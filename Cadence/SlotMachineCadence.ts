...
function slotCadence(symbols: Array<SlotCoordinate>): SlotCadence {
  ...
  const cadences: SlotCadence = new Array(columnSize);
  cadences[0] = 0;
  
  const countMap = new Int32Array(columnSize);
  for (let i = 0; i < symbols.length; i++) {
    const col = symbols[i].column;
    if (col < columnSize) countMap[col]++;
  }

  let cumulativeSpecialSymbols = 0;
  for (let i = 1; i < columnSize; i++) {
    cumulativeSpecialSymbols += countMap[i - 1];

    const isAnticipating =
      cumulativeSpecialSymbols >= minToAnticipate &&
      cumulativeSpecialSymbols < maxToAnticipate;

    const interval = isAnticipating ? anticipateCadence : defaultCadence;
    cadences[i] = cadences[i - 1] + interval;
  }
